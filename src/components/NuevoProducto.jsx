...
export default function NuevoProducto({ onClose, codigoPreestablecido }) { 
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await fetch('http://18.119.160.177:3001/api/products', {
        method: 'POST',
        ...
        if (validPrices.length === 0) {
            ...
            if (data.items && data.items[0]?.link) {
                ...
                try {
                    ...
                    const scrapeResponse = await fetch('http://18.119.160.177:3001/api/scrape-price', { // Asegúrate que la URL del backend sea correcta
                        method: 'POST',
                        headers: { 'Content-Type': 'application/json' },
