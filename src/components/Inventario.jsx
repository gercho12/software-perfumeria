...
export default function Inventario() {
  ...
  const fetchProducts = useCallback(async () => {
    try {
      const url = new URL('http://18.119.160.177:3001/api/products');
  ...
  const updateStock = async (producto, newStock) => {
    try {
      const response = await fetch(`http://18.119.160.177:3001/api/products/${producto.id}`, {
        method: 'PUT',
  ...
  const saveEdit = async () => {
    if (editingProduct) {
      try {
        const response = await fetch(`http://18.119.160.177:3001/api/products/${editingProduct.id}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
